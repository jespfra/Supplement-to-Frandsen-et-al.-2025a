...
mutable struct SolverCache
	...
	# For SMB setups, one-column-analog using fixed-point iteration (OCA-FPI) to achieve cyclic steady state (CSS) is possible - https://doi.org/10.1016/j.compchemeng.2017.12.022
	css::Bool											# Activates cyclic steady state
	oca_fpi::Bool 										# Activates oca_fpi, defaults to false 
	css_max_iter::Int64 								# Set max number of iterations 
	css_tol::Union{Float64, Int64} 						# Set cyclic steady state tolerance between two cycles 
	
	idx_units::Vector{Int64}
	nColumns::Int64
	# the ODE solver cannot be stored because when changing the solver, the datatype is also changed..

	function SolverCache(; columns::Union{Tuple,ModelBase}, switches::Switches, outlets::Union{Tuple,CreateOutlet} = (0,), x0 = [0], abstol=1e-12, reltol=1e-10, dt=1.0, solution_times=[0], prototypeJacobian=true, analyticalJacobian=false, css=false, oca_fpi=false, css_max_iter=200, css_tol=1e-4)
		...
		new(x0, abstol, reltol, dt, solution_times, prototypeJacobian, analyticalJacobian, css, oca_fpi, css_max_iter, css_tol, idx_units, nColumns)
	end
end
