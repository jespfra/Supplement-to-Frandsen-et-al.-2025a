...
function create_units(model::Union{Dict, OrderedDict})
            ...
            if unit_type == "INLET"
                inlet_instance = CreateInlet(
					...
					for key1 = 1:length(keys(value))
                        ID = num_sections < 10 ? "sec_00$(num_sections-1)" : num_sections < 100 ? "sec_0$(num_sections-1)" : "sec_$(num_sections-1)"
                        # ID = "sec_00$(num_sections-1)"
						# If inlets are specified 
						if ID in keys(value)
							
							# Format the number iterator with leading zeros
                            num_iterator = num_sections < 10 ? "00$(num_sections-1)" : num_sections < 100 ? "0$(num_sections-1)" : "$(num_sections-1)"
							# if num_sections<10
    ...
    for i = 0:model["root"]["input"]["model"]["connections"]["nswitches"]-1 #i = 0
        ...
        # end
        switchIterator = i < 10 ? "switch_00$(i)" : i < 100 ? "switch_0$(i)" : "switch_$(i)"
		...
		for j = 1:size(connectionMatrix)[1] #j=1
			...
			sourceID = ID < 10 ? "unit_00$ID" : ID < 100 ? "unit_0$ID" : "unit_$ID"
            ID = Int64(connectionMatrix[j,2])
            sinkID = ID < 10 ? "unit_00$ID" : ID < 100 ? "unit_0$ID" : "unit_$ID"
    ...
    # If OCA-FPI is activated
	if haskey(model["root"]["input"]["solver"],"css")
		css = haskey(model["root"]["input"]["solver"]["css"], "activate") ? model["root"]["input"]["solver"]["css"]["activate"] : false
		oca_fpi = haskey(model["root"]["input"]["solver"]["css"], "oca_fpi") ? model["root"]["input"]["solver"]["css"]["oca_fpi"] : false
		css_max_iter = haskey(model["root"]["input"]["solver"]["css"], "css_max_iter") ? model["root"]["input"]["solver"]["css"]["css_max_iter"] : 200
		css_tol = haskey(model["root"]["input"]["solver"]["css"], "css_tol") ? model["root"]["input"]["solver"]["css"]["css_tol"] : 1e-4
	else 
		css = false
		oca_fpi = false 
		css_max_iter = 1000
