...
function solve_model(; columns, switches::Switches, solverOptions, outlets=(0,), alg=QNDF(autodiff=false))
	...
	# if css is activated, special treatment is required 
	# Hence, this activates another script 
	if solverOptions.css == true 
		...
		return 	evals, l2norm, l2norm_anzalytical, maxE
	end
	...
	x0 = solverOptions.x0
	#running simulations
	for i = 1: length(switches.section_times) - 1 # corresponds to sections i=12
