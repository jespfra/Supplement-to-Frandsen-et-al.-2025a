...
# A function that determines the inlet concentrations needed for the transport equations 
# Depends on the inlets specified in the switches
function inlet_concentrations!(cIn, switches, j, section, sink, x, t, inlet_type::DynamicInlets, poly_input)
	"""
		A function that determines the inlet concentrations needed for the transport equations. 
		Depends on the inlets specified in the switches. 
		input is: 
	...
	@inbounds for l in eachindex(switches.ConnectionInstance.c_connect[switches.switchSetup[section]][sink][j])
		# println("section = $section, sink = $sink, j = $j, l = $l")
		cIn[j] += switches.ConnectionInstance.u_unit[switches.switchSetup[section]][sink][l] * switches.ConnectionInstance.c_connect[switches.switchSetup[section]][sink][j][l] * poly_input[j](t)
	end 
...
function compute_transport_oca_fpi!(RHS, RHS_q, cpp, x, m::LRM, t, section, sink, switches, idx_units, poly_input) 
	"""
	Define a function to compute the transport term for the LRM but the input is evaluated from interpolating polynomials 
	This is used in the one-column analog fixed point iteration where the chromatogram from one column is used as input into the next 
	"""
	...
	@inbounds for j = 1:m.nComp
		...
		# Determining inlet concentration 
		inlet_concentrations!(m.cIn, switches, j, section, sink, x, t, switches.inlet_conditions[section, sink, j], poly_input) 
		# m.cIn[j] = ((switches.ConnectionInstance.cIn_c[section][sink][j] + 
		...
		# 			switches.ConnectionInstance.u_unit[switches.switchSetup[section], sink] * switches.ConnectionInstance.c_connect[switches.switchSetup[section], sink, j] * poly_input[j](t)) / switches.ConnectionInstance.u_tot[switches.switchSetup[section], sink]
...
function compute_transport_oca_fpi!(RHS, RHS_q, cpp, x, m::LRMP, t, section, sink, switches, idx_units, poly_input)
	"""
	Define a function to compute the transport term for the LRMP but the input is evaluated from interpolating polynomials 
	This is used in the one-column analog fixed point iteration where the chromatogram from one column is used as input into the next 
	"""
...
function compute_transport_oca_fpi!(RHS, RHS_q, cpp, x, m::GRM, t, section, sink, switches, idx_units, poly_input)
	"""
	Define a function to compute the transport term for the GRM but the input is evaluated from interpolating polynomials 
	This is used in the one-column analog fixed point iteration where the chromatogram from one column is used as input into the next 
	"""
