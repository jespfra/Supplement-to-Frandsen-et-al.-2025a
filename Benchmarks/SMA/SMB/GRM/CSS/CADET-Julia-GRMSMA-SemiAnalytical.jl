...
function model_setup(Q2,Q4,QF,QD,QE,QR,ts,nCells, polyDeg, polyDegPore, exactInt, analJac=false)
    ...
    model["root"]["input"]["solver"]["css"]["css_tol"] = 1e-5           # Set cyclic steady state tolerance between two cycles 
...
inlets, outlets, columns, switches, solverOptions = model_setup(Q2,Q4,QF,QD,QE,QR,ts, 8, 6, 12, 1)
...
df = DataFrame(C0_E = outlets[1].solution_outlet[end - length(solverOptions.solution_times):end,1], C1_E = outlets[1].solution_outlet[end - length(solverOptions.solution_times):end,2], C2_E = outlets[1].solution_outlet[end - length(solverOptions.solution_times):end,3], C3_E = outlets[1].solution_outlet[end - length(solverOptions.solution_times):end,4],
               C0_R = outlets[2].solution_outlet[end - length(solverOptions.solution_times):end,1], C1_R = outlets[2].solution_outlet[end - length(solverOptions.solution_times):end,2], C2_R = outlets[2].solution_outlet[end - length(solverOptions.solution_times):end,3], C3_R = outlets[2].solution_outlet[end - length(solverOptions.solution_times):end,4])
# CSV.write(joinpath(@__DIR__,"Semi-analytical_GRM_SMA_CSS.csv"), df)  
