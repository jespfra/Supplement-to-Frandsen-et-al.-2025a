...
c_analytical = CSV.read((joinpath(@__DIR__,"FVSemi-analytical_GRM_SMA.csv")),DataFrame)



function model_setup(Q1,Q2,Q3,Q4,QF,QD,QE,QR,ts,nCells, polyDeg, polyDegPore, exactInt, analJac=false)
...
inlets, outlets, columns, switches, solverOptions = model_setup(Q1,Q2,Q3,Q4,QF,QD,QE,QR,ts, 4, 4, 4, 1)
...
plot(outlets[1].solution_outlet[:,2], label="Component 1")
plot!(c_analytical[!,"C1_E"])

# Evaluate the convergence using the evaluate_convergence function 
evaluate_convergence(Q1,Q2,Q3,Q4,QF,QD,QE,QR,ts,QNDF(autodiff=false), c_analytical, nComp, nCell, polyDeg, polyDegPore, "GRM", @__DIR__)
