...
#Setting up the model
    ...
    smb_model.root.input.model.unit_004.unit_type = 'GENERAL_RATE_MODEL'
    ...
    smb_model.root.input.model.unit_004.col_dispersion = 5.75e-8
    smb_model.root.input.model.unit_004.col_length = 1.4e-2
    #smb_smb_model.root.input.model.unit_004.total_porosity = 0.37+0.75*(1-0.37)
    smb_model.root.input.model.unit_004.cross_section_area = 1

    smb_model.root.input.model.unit_004.film_diffusion = [6.9e-6,6.9e-6, 6.9e-6, 6.9e-6]
    smb_model.root.input.model.unit_004.par_diffusion = [70e-11, 6.07e-11, 6.07e-11, 6.07e-11]
    smb_model.root.input.model.unit_004.par_surfdiffusion = [0.0,0.0,0.0,0.0]
    ...
    smb_model.root.input.model.unit_004.discretization.ncol = ncol #Should be high enough 
    smb_model.root.input.model.unit_004.discretization.npar = ncolPore #Should be high enough 
    ...
    smb_model.root.input.model.unit_000.sec_000.const_coeff = [282, 1, 1, 1] #Inlet flowrate concentration
    smb_model.root.input.model.unit_001.sec_000.const_coeff = [282, 0, 0, 0] #Desorbent stream
...
c_analytical = pd.read_csv('FVSemi-analytical_GRM_SMA.csv')
...
for l in range(0,len(k)):
    ...
    rtimes = [0,0,0]
    for i in range(1):
        df,rtime = model(2**k[l],2**(k[l]-2))
        rtimes[i] = rtime

    runtime.append(rtimes[0])
    print(k[l])
    ncomp = int((df.shape[1]-1)/2)
    for m in range(ncomp): #Number of components
        idxxE = f'C{m}_E'
        idxxR = f'C{m}_R'
        err = max([err,abs(df[idxxE][:] - c_analytical[idxxE][:]).max()])
        err = max([err,abs(df[idxxR][:] - c_analytical[idxxR][:]).max()])
    maxE_e.append(err)
    DOF.append(ncomp*2**k[l]*4 + ncomp*2*2**(k[l]-2)*4)  # Four components, two phases, four columns
